...
from src.websocket_ip4_to_ip6.client import LANGameLink_426_client as Client

class ClientFrame(tk.Frame):
    def __init__(self, master=None, **kwargs):
        super().__init__(master, **kwargs)
        self.master = master
        self.client = Client(self.update_status)
        self.service_thread = None
        self.new_loop = None
        self.delay_update_id = None
        
        # 初始化动态端口字典
        self.dynamic_ports = {"TCP": self.client.GAME_TCP_PORTS, "UDP": self.client.GAME_UDP_PORTS}

        
        
        # 初始化 IPv6 地址和端口
        self.inputs = [
            tk.Label(self, text="第一步：填写主机信息")
        ]
        self.ipv6_address_input = LabeledInput(self, "主机IPv6地址（必填）", self.client.SERVER_IPV6_ADDRESS, InputValidator.validate_ipv6)
        self.ipv6_port_input = LabeledInput(self, "主机IPv6端口（必填）", self.client.SERVER_PORT, InputValidator.validate_port)
       ...
       # 添加一个空行
        ...
        # 添加本地监听的 IP 地址
        self.ipv4_address_input = LabeledInput(self, "本地监听IP4", self.client.GAME_IPV4_ADDRESS, InputValidator.validate_ipv4,readonly=True)
        ...
        btn1 = tk.Button(btn_frame, text="连接主机", command=self.start_services, width=10)
    ...
    def _update_client_attr(self):
        self.client.SERVER_IPV6_ADDRESS = self.ipv6_address_input.var.get()
        self.client.SERVER_PORT = int(self.ipv6_port_input.var.get())
        self.client.GAME_IPV4_ADDRESS = self.ipv4_address_input.var.get()
        TCP_PORTS = []
        UDP_PORTS = []
        # 更新动态端口的值
    ...
    def start_services(self):
        ...
        # 将我们的协程安排到新的事件循环中
        asyncio.run_coroutine_threadsafe(self.client.run(), self.new_loop)
